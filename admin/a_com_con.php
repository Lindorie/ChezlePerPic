...
<?php 
	if (isset($_GET['outils']) AND $_GET['outils'] == 'voirmsgenvoyes') {
		// MESSAGES ENVOYES
		$rq = 'SELECT titre, identifiant, consultants_msg.id, DATE_FORMAT(date, "%d/%m/%Y %H:%i") as date, lecture FROM consultants_msg, user WHERE auteur = '.$_SESSION['id'].' AND parent = "" AND user.id = consultants_msg.auteur ORDER BY date ASC;';
