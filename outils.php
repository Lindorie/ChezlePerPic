				...
				if(isset($_POST['submit'])) {
					...
					if ($type == 'activites') {
						...
						if ($_FILES['image']['size'] != 0) {
							...
							$rq = 'INSERT INTO '.$prefix.'activites(titre,accroche,texte,categorie,photo) VALUES( "'.$titre.'", "'.$accroche.'", "'.$texte.'", "'.$cat.'", "'.$nom_image.'")';
						} else {
							$rq = 'INSERT INTO '.$prefix.'activites(titre,accroche,texte,categorie) VALUES( "'.$titre.'", "'.$accroche.'", "'.$texte.'", "'.$cat.'")';
						}
					} 
					else if ($type == 'news') {
						$rq = 'INSERT INTO '.$prefix.'news(titre,accroche,texte,date) VALUES( "'.$titre.'", "'.$accroche.'", "'.$texte.'", CURDATE())';
					} else if ($type == 'livredor') {
						$rq = 'INSERT INTO '.$prefix.'livre_dor(auteur,mois_loc,texte) VALUES( "'.$auteur.'", "'.$mois_loc.'", "'.$texte.'")';
					} else {
						$page = $_REQUEST['page'];
						$rq = 'INSERT INTO '.$prefix.'content(titre,texte,page) VALUES ("'.$titre.'", "'.$texte.'", "'.$page.'")';
					}
